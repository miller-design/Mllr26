 ...
 *
 * Fetches image paths via `/api/projects/{slug}/images` based on the `slug`
 * prop (project folder name under `public/projects`).
 *
...
const { data: images } = await useFetch<string[]>(
  () => `/api/projects/${slug}/images`,
);
</script>
